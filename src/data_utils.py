...
def revert_output_format(poses, data_mean, data_std, dim_to_ignore):
  ...
  for i in xrange(poses_out.shape[0]):
    poses_out_list.append(
      unNormalizeData(poses_out[i, :, :], data_mean, data_std, dim_to_ignore))
...
def load_data(path_to_dataset, actions):
  ...
  for action_idx in np.arange(len(actions)):
    ...
    for fn in os.listdir(path):
      count = count+1
    for examp_index in np.arange(count):  # subactions

      #print("Reading subject {0}, action {1}, subaction {2}".format(subj, action, subact))

      filename = '{0}/{1}/{2}_{3}.txt'.format( path_to_dataset, action, action, examp_index+1)
      ...
      trainData[(action, examp_index+1, 'even')] = action_sequence[even_list, :]
